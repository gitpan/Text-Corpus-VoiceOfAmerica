...
use Test::More;
BEGIN { use_ok( 'Text::Corpus::VoiceOfAmerica' ); }
BEGIN { use_ok( 'Text::Corpus::VoiceOfAmerica::Document' ); }
...
# make sure a Text::Corpus::VoiceOfAmerica is returned.
...
if (defined (%ENV) && exists ($ENV{TEXT_CORPUS_VOICEOFAMERICA_FULL_TESTING}) && $ENV{TEXT_CORPUS_VOICEOFAMERICA_FULL_TESTING})
{
    ...
    eval {
      ...
      isa_ok ($document, 'Text::Corpus::VoiceOfAmerica::Document');
    ...
    {
      ok ((@ignore == 7), 'Parsing completed.');
    }
}
else
{
  diag ("\nTests needing network access skipped.\nDefine environment variable\nTEXT_CORPUS_VOICEOFAMERICA_FULL_TESTING to 1\nto run complete tests.\n");
}
